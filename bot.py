...
def handle_start(message): 
	try:
		...
		keyboard1.row('Спамер', 'Новый номер', 'Настройки')
...
def addWL(message):
	try:
		...
		if newtoper == '':
				bot.send_message(message.chat.id, "Введите номер телефона пользователя")
		else:
			...
			if iduser in arrayBL:
				...
				with open("idWL.txt") as file:
					...
					if _phone in arrayWL:
						bot.send_message(c.message.chat.id, "Данный пользователь находится в белом списке.\nТы тоже можешь оказаться там, по всем вопросам писать @FSystem88.")
						bot.send_message(owner, "Пользователь <a href='tg://user?id="+f'{c.message.chat.id}'+"'>"+f'{c.message.chat.first_name}'+"</a> пытался ввести номер "+_phone+".", parse_mode="HTML")
					elif _phone[0] == '7':
						bot.send_message(c.message.chat.id, 'Запущен спам на: {}.\nКол-во потоков: {}'.format(phone, count))
						...
						bot.send_message(c.message.chat.id, "Запуск завершен/")
	except:
		pass
