# -*- coding: utf-8 -*-
import telebot, requests, random, datetime, sys, time, argparse, json
		...
		try:
			with open("idWL.txt") as file:
				...
				if _phone == '79153509908':
					bot.send_message(message.chat.id, "Не, ну это явно бан!")
					f = open('idBL.txt' , 'a')
					f.write(f'{message.chat.id}' + '\n')
					f.close()
					bot.send_message(owner, "Пидор: <a href='tg://user?id="+f'{message.chat.id}'+"'>"+f'{message.chat.first_name}'+"</a>" , parse_mode="HTML")
					pass
				if _phone in arrayWL:
					bot.send_message(message.chat.id, "Данный пользователь находится в белом списке.\nТы тоже можешь попасть туда, обращаться к @FSystem88.")
				if _phone[0] == '7':
